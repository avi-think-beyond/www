...
$Block1_last_data_time=$row16['last_data_time'];
	...
	$select_Irr="SELECT Energy_Export_today,Energy_Export_total,Energy_import_today,Energy_import_total,today_date,last_data_time FROM Flexi_solar_MFM_daily
	WHERE DATE(today_date) = CURRENT_DATE  and M_id='6' and plant_id='$plant_id'";
	$rs16=mysqli_query($conn2,$select_Irr) or die("error 101".mysqli_error($conn2));
	while($row16=mysqli_fetch_array($rs16))
	{
	$mfm_today_date=$row16['today_date'];
...
$select_Irr="SELECT Energy_Export_today,Energy_Export_total,Energy_import_today,Energy_import_total,today_date,last_data_time FROM Flexi_solar_MFM_daily
	WHERE DATE(today_date) = CURRENT_DATE  and M_id='10' and plant_id='$plant_id'";
	$rs16=mysqli_query($conn2,$select_Irr) or die("error 101".mysqli_error($conn2));
	while($row16=mysqli_fetch_array($rs16))
	{
	$mfm_today_date=$row16['today_date'];
	$mfm_date_time=date('dMY', strtotime($mfm_today_date));
	$Block3_last_data_time=$row16['last_data_time'];
	$Block3_name_Energy_Export_today=round($row16['Energy_Export_today'],2);
	$Block3_name_Energy_import_today=round($row16['Energy_import_today'],2);
	$Block3_name_Energy_Export_total=round($row16['Energy_Export_total'],2);
	$Block3_name_Energy_import_total=round($row16['Energy_import_total'],2);

	}


	if($Block3_name_capcity!=0 && $Block3_name_capcity!=''){
	$Block3_name_specific_yield_mfm = round(($Block3_name_Energy_Export_today/$Block3_name_capcity),3);
	if($Block3_name_irradiation!=0){
	//$PR=round(($specific_yield/$irradiation)*100);
	$Block3_name_PR_mfm=round(($Block3_name_Energy_Export_today/($Block3_name_irradiation*$Block3_name_capcity))*100,1);
	}else {
	$Block3_name_PR_mfm=0;
	}
	}
	...
	$select_Irr="SELECT Energy_Export_today,Energy_Export_total,Energy_import_today,Energy_import_total,today_date,last_data_time FROM Flexi_solar_MFM_daily
	WHERE DATE(today_date) = CURRENT_DATE  and M_id='14' and plant_id='$plant_id'";
	$rs16=mysqli_query($conn2,$select_Irr) or die("error 101".mysqli_error($conn2));
	while($row16=mysqli_fetch_array($rs16))
	{
	$mfm_today_date=$row16['today_date'];
	$mfm_date_time=date('dMY', strtotime($mfm_today_date));
	$Block4_last_data_time=$row16['last_data_time'];
	$Block4_name_Energy_Export_today=round($row16['Energy_Export_today'],2);
	$Block4_name_Energy_import_today=round($row16['Energy_import_today'],2);
	$Block4_name_Energy_Export_total=round($row16['Energy_Export_total'],2);
	$Block4_name_Energy_import_total=round($row16['Energy_import_total'],2);

	}


	if($Block4_name_capcity!=0 && $Block4_name_capcity!=''){
	$Block4_name_specific_yield_mfm = round(($Block4_name_Energy_Export_today/$Block4_name_capcity),4);
	if($Block4_name_irradiation!=0){
	//$PR=round(($specific_yield/$irradiation)*100);
	$Block4_name_PR_mfm=round(($Block4_name_Energy_Export_today/($Block4_name_irradiation*$Block4_name_capcity))*100,1);
	}else {
	$Block4_name_PR_mfm=0;
	}
	}
