...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('BA2:BD2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('BE2:BH2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:BH2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:BH')->getFont()->setBold(true)->setSize(10);
 $objPHPExcel->getActiveSheet()->getStyle('A:BH')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('AY2', 'MCR1 Inverter2 (KWh)')
								...
								->setCellValue('BA2'.$row, 'OG_MFM- Enery Export (KWh)')
								->setCellValue('BB2'.$row, 'OG_MFM - Enery Import (KWh)')
								->setCellValue('BC2'.$row, 'OG_MFM - Enery Export Till date (KWh)')
								->setCellValue('BD2'.$row, 'OG_MFM- Enery Import Till date (KWh)')
                              
								->setCellValue('BE2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('BF2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('BG2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('BH2'.$row, 'Performance ratio (PR)');
...
$kwh_today='';
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('BE'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('BF'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('BH'.$row, $PR);
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->mergeCells('A1:BH1');
$objPHPExcel->getActiveSheet()->setTitle(date('F-H'));
