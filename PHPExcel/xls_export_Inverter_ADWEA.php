...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('A2:J2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E6B9B8'))))->getAlignment();
 $objPHPExcel->getActiveSheet()->getStyle('K2:R2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('S2:V2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('B2', 'Inverter 1 (KWh)')
                ->setCellValue('C2', 'Inverter 2 (KWh)')
                ->setCellValue('D2', 'Inverter 3 (KWh)')
                ->setCellValue('E2', 'Inverter 4 (KWh)')
                ->setCellValue('F2', 'Inverter 5 (KWh)')
                ->setCellValue('G2', 'Inverter 6 (KWh)')
                ->setCellValue('H2', 'Inverter 7 (KWh)')
                ->setCellValue('I2', 'Inverter 8 (KWh)')
                ...
                // ->setCellValue('AY2', 'Inverter 50 (KWh)')
							      ...
							      ->setCellValue('J2', 'Total (KWh)')
								->setCellValue('K2'.$row, 'MFM 1 - Enery Export (KWh)')
								->setCellValue('L2'.$row, 'MFM 1 - Enery Import (KWh)')
								->setCellValue('M2'.$row, 'MFM 1 - Enery Export Till date (KWh)')
								->setCellValue('N2'.$row, 'MFM 1 - Enery Import Till date (KWh)')
                                ->setCellValue('O2'.$row, 'MFM 2 - Enery Export (KWh)')
                                ->setCellValue('P2'.$row, 'MFM 2 - Enery Import (KWh)')
                                ->setCellValue('Q2'.$row, 'MFM 2 - Enery Export Till date (KWh)')
                                ->setCellValue('R2'.$row, 'MFM 2 - Enery Import Till date (KWh)')
								->setCellValue('S2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('T2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('U2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('V2'.$row, 'Performance ratio (PR)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row, $ENERGY_Generated_today);
$server_date_time='';
$Meter_id='';
$kwh_today='';
	$row++;
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('T'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$Energy_Export_total=round($row16['Energy_Export_total']/1000,2);
$Energy_import_total=round($row16['Energy_import_total']/1000,2);
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$row, $Energy_import_total);
...
// $PR=0;  
// }
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('P'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$row, $Energy_import_total);
$Energy_Export_today='';
$Energy_import_today='';
$server_date_time='';
$Meter_id='';
...
$Energy_Export_today=round($row16['sum_EE']/1000,2);
//$solar_plant_dc_capacity=5600;
$specific_yield = round(($Energy_Export_today/$solar_plant_dc_capacity),2);
if($irr_today!=0){
//$PR=round(($specific_yield/$irr_today)*100);
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('V'.$row, $PR);
