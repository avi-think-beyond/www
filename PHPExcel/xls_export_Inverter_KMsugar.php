...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('A2:G2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E6B9B8'))))->getAlignment();
 $objPHPExcel->getActiveSheet()->getStyle('H2:K2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('L2:O2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:O2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:O')->getFont()->setBold(true)->setSize(10);
 $objPHPExcel->getActiveSheet()->getStyle('A:O')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('F2', 'Inverter 5 (KWh)')
							      ...
							      ->setCellValue('G2', 'Total (KWh)')
								->setCellValue('H2'.$row, 'OG_MFM - Enery Export (KWh)')
								->setCellValue('I2'.$row, 'OG_MFM - Enery Import (KWh)')
								->setCellValue('J2'.$row, 'OG_MFM - Enery Export Till date (KWh)')
								->setCellValue('K2'.$row, 'OG_MFM - Enery Import Till date (KWh)')
                          
								->setCellValue('L2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('M2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('N2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('O2'.$row, 'Performance ratio (PR)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$row, $ENERGY_Generated_today);
$server_date_time='';
$Meter_id='';
$kwh_today='';
	$row++;
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$row, $Energy_import_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$row, $PR);
$Energy_Export_today='';
$Energy_import_today='';
$server_date_time='';
$Meter_id='';
...
$objPHPExcel->getActiveSheet()->setTitle(date('F-O'));
