...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('AQ2:AX2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('AY2:BB2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:BB2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:BB')->getFont()->setBold(true)->setSize(10);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('AO2', 'Block10 Inverter4 (KWh)')
								...
								->setCellValue('AY2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('AZ2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('BA2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('BB2'.$row, 'Performance ratio (PR)');
...
$kwh_today='';
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('AY'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('AZ'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
//print_r($irrarr);
$select_Irr="SELECT Energy_Export_today,Energy_Export_total,Energy_import_today,Energy_import_total,today_date FROM Flexi_solar_MFM_daily 
WHERE MONTH(today_date) = MONTH(CURRENT_DATE) AND YEAR(today_date) = YEAR(CURDATE())  and M_id='12' and plant_id='$plant_id' order by today_date DESC";
$rs16=mysqli_query($conn2,$select_Irr) or die("error 101".mysqli_error($conn2));
while($row16=mysqli_fetch_array($rs16))
{
$mfm_today_date=$row16['today_date'];
...
$server_date_time=date('d-M-Y', strtotime($mfm_today_date));
 ...
 $objPHPExcel->setActiveSheetIndex(0)->setCellValue('BA'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('BB'.$row, $PR);
