...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('A2:Q2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E6B9B8'))))->getAlignment();
 $objPHPExcel->getActiveSheet()->getStyle('R2:U2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('V2:Y2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:Y2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:Y')->getFont()->setBold(true)->setSize(10);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
               ...
               ->setCellValue('B2', 'Block1 Inverter1 (KWh)')
                ...
                // ->setCellValue('Q2', 'Inverter 14 (KWh)')
                // ->setCellValue('R2', 'Inverter 15 (KWh)')
                ...
                // ->setCellValue('AY2', 'Inverter 50 (KWh)')
							      ...
							      ->setCellValue('Q2', 'Total (KWh)')
								->setCellValue('R2'.$row, 'OG1 MFM - Enery Export (KWh)')
								->setCellValue('S2'.$row, 'OG1 MFM - Enery Import (KWh)')
								->setCellValue('T2'.$row, 'OG1 MFM - Enery Export Till date (KWh)')
								->setCellValue('U2'.$row, 'OG1 MFM - Enery Import Till date (KWh)')
                               
								->setCellValue('V2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('W2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('X2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('Y2'.$row, 'Performance ratio (PR)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$row, $ENERGY_Generated_today);
$server_date_time='';
$Meter_id='';
$kwh_today='';
	$row++;
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('V'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('W'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('T'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('U'.$row, $Energy_import_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('X'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Y'.$row, $PR);
$Energy_Export_today='';
$Energy_import_today='';
$server_date_time='';
$Meter_id='';
