...
$solar_plant_name_replace_space = str_replace(" ", "_", 'ALL_Inverter');
...
$plant_name=array();
$cnt=1;
$qry="select * from Flexi_solar_plant_info where status=1";
 $result=mysqli_query($conn2,$qry) or print("error 412".mysqli_error($conn2));
while($fetch=mysqli_fetch_array($result)){
        $plant_id[]=$fetch['id'];
        $plant_name[] =$fetch['plant_name'];
        $cnt++;
      }

for ($one=0; $one <=39; $one++) { 
$solar_plant_id=$plant_id[$one];
...
include'ALL_Inverter.php';
$objPHPExcel->getActiveSheet()->setTitle(substr($plant_name[$one],0,30));
