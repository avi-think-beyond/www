...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('A2:E2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E6B9B8'))))->getAlignment();
 $objPHPExcel->getActiveSheet()->getStyle('F2:I2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('J2:M2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:M2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:M')->getFont()->setBold(true)->setSize(10);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('D2', 'Inverter3 (KWh)')
							      ...
							      ->setCellValue('E2', 'Total (KWh)')
								->setCellValue('F2'.$row, 'OG_MFM - Enery Export (KWh)')
								->setCellValue('G2'.$row, 'OG_MFM - Enery Import (KWh)')
								->setCellValue('H2'.$row, 'OG_MFM - Enery Export Till date (KWh)')
								->setCellValue('I2'.$row, 'OG_MFM - Enery Import Till date (KWh)')
                          
								->setCellValue('J2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('K2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('L2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('M2'.$row, 'Performance ratio (PR)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$row, $ENERGY_Generated_today);
$server_date_time='';
$Meter_id='';
$kwh_today='';
	$row++;
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$row, $Energy_import_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$row, $PR);
$Energy_Export_today='';
$Energy_import_today='';
$server_date_time='';
$Meter_id='';
...
$objPHPExcel->getActiveSheet()->setTitle(date('F-M'));
