...
$objPHPExcel = new PHPExcel();
 ...
 $objPHPExcel->getActiveSheet()->getStyle('A2:F2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E6B9B8'))))->getAlignment();
 $objPHPExcel->getActiveSheet()->getStyle('G2:J2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'FFFF00'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('K2:N2')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'E4AD7B'))))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
 $objPHPExcel->getActiveSheet()->getStyle('A2:N2')->getFont()->setBold(true)->setSize(11);
 $objPHPExcel->getActiveSheet()->getStyle('A:N')->getFont()->setBold(true)->setSize(10);
...
->setCellValue('A1',date("l jS \of F Y").' ENERGY PRODUCTION')
                ...
                ->setCellValue('E2', 'Inverter 4 (KWh)')
							      ...
							      ->setCellValue('F2', 'Total (KWh)')
								->setCellValue('G2'.$row, 'OG_MFM - Enery Export (KWh)')
								->setCellValue('H2'.$row, 'OG_MFM - Enery Import (KWh)')
								->setCellValue('I2'.$row, 'OG_MFM - Enery Export Till date (KWh)')
								->setCellValue('J2'.$row, 'OG_MFM - Enery Import Till date (KWh)')
                          
								->setCellValue('K2'.$row, 'Irradiation today [GHI] (kWh/m2)')
								->setCellValue('L2'.$row, 'Irradiation today [DNI](kWh/m2)')
								->setCellValue('M2'.$row, 'Specific Yield (kWh/kWp)')
								->setCellValue('N2'.$row, 'Performance ratio (PR)');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$row, $ENERGY_Generated_today);
$server_date_time='';
$Meter_id='';
$kwh_today='';
	$row++;
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$row, $irradiation);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$row, $irradiation_tilt1);
	$server_date_time='';
	$Meter_id='';
	$kwh_today='';
	 $irradiation='';
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$row, $Energy_Export_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$row, $Energy_import_today);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$row, $Energy_Export_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row, $Energy_import_total);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$row, $specific_yield);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$row, $PR);
$Energy_Export_today='';
$Energy_import_today='';
$server_date_time='';
$Meter_id='';
...
$objPHPExcel->getActiveSheet()->setTitle(date('F-N'));
